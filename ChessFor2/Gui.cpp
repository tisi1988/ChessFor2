...
std::pair<int, int> screenToWindowCoordinates(SDL_Window* boardWindow, int x, int y) {
    int windowX, windowY;
    SDL_GetWindowPosition(boardWindow, &windowX, &windowY);
    return {x - windowX, y - windowY};
}

std::pair<int, int> windowCoordinateToTile(SDL_Window* boardWindow, int x, int y) {
    int windowW, windowH;
...
Gui::Gui(std::function<void (Position)> const& tileClickCb, std::function<void ()> const& exitCb) {
    ...
    auto rawClickCb = [this, tileClickCb](int x, int y){
        ...
        // to a tile
        auto [clickX, clickY] = screenToWindowCoordinates(m_render->getBoardWindow(), x, y);
        auto [tileCol, tileRow] = windowCoordinateToTile(m_render->getBoardWindow(), clickX, clickY);
